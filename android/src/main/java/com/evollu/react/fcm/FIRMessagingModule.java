...
public class FIRMessagingModule extends ReactContextBaseJavaModule implements LifecycleEventListener, ActivityEventListener {
        ...
        LocalBroadcastManager.getInstance(getReactApplicationContext()).registerReceiver(new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                if (getReactApplicationContext().hasActiveCatalystInstance()) {
                    ...
                    SenkronizasyonHelper SenkHelper = new SenkronizasyonHelper(getReactApplicationContext());
                    SenkHelper.updateSenkronizasyon();
    ...
    private void sendLocaleNotify()
    {
        Bundle bndl = new Bundle();
        bndl.putString("title","Zorunlu Güncelleme Talebi");
        bndl.putString("body","Yeni bir zorunlu güncelleme talebiniz mevcut.");
        bndl.putString("show_in_foreground","true");
        bndl.putString("sound","default");
        mFIRLocalMessagingHelper.sendNotification(bndl);
    }
